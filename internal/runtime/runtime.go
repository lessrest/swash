...
func (r *Runtime) ListSessions(ctx context.Context) ([]Session, error) {
	statuses, err := r.Processes.List(ctx, process.ProcessFilter{
	...
	for _, st := range statuses {
		...
		sessions = append(sessions, Session{
			ID:      st.Ref.SessionID,
			Unit:    unitNameStringForRef(st.Ref),
			PID:     st.PID,
			CWD:     st.WorkingDir,
			Status:  status,
...
func (r *Runtime) StopSession(ctx context.Context, sessionID string) error {
	return r.Processes.Stop(ctx, process.HostProcess(sessionID))
}
...
func (r *Runtime) PollSessionOutput(sessionID, cursor string) ([]Event, string, error) {
	filters := []EventFilter{eventlog.FilterBySession(sessionID)}
...
func (r *Runtime) ListHistory(ctx context.Context) ([]HistorySession, error) {
	...
	filters := []EventFilter{eventlog.FilterByEvent(eventlog.EventExited)}
