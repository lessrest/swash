...
func main() {
	...
	srv2, err := gmail.NewService(ctx, option.WithHTTPClient(client))
	if err != nil {
		log.Fatalf("Unable to retrieve Gmail client: %v", err)
	}

	user := "me"
	r, err := srv2.Users.Threads.List(user).MaxResults(10).Do()
	if err != nil {
	...
	// iterate to fetch each thread
	for _, thread := range r.Threads {
		msg, err := srv2.Users.Messages.Get(user, thread.Id).Do()
		if err != nil {
